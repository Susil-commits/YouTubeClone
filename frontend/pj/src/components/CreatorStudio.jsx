...
function CreatorStudio({ onLogout, openCreateOnLoad = false, onClearCreateOpen }) {
  ...
  const [, setBannerProgress] = useState(0);
  ...
  const startUpload = useCallback(async (fileArg) => {
    const selected = fileArg || file;
    if (!selected) return;
    setUploading(true);
    setProgress(0);
    try {
      const res = await uploadVideoFile(selected, setProgress);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropAccepted: (files) => {
      ...
      setStep(2);
      try { startUpload(f); } catch (e) { void e; }
    }
  });
  ...
  // startUpload defined with useCallback above
                                        ...
                                        <div className="flex items-center gap-2 mb-1">
                                            {(() => {
                                                ...
                                                try {
                                                  ...
                                                  lc = Number(likes[v._id] || 0);
                                                  dc = Number(dislikes[v._id] || 0);
                                                } catch (e) { void e; }
                                                return (
                                                  <>
                                                    <span className="px-2 py-0.5 rounded-full text-[10px] bg-green-600/20 text-green-300 border border-green-500/30">Likes {lc}</span>
                                                    <span className="px-2 py-0.5 rounded-full text-[10px] bg-red-600/20 text-red-300 border border-red-500/30">Dislikes {dc}</span>
                                                  </>
                                                );
                                            })()}
                                        </div>
