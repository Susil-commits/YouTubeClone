...
function AdminVideoManager({ onLogout }) {
  ...
  async function _setApproved(v, approved) {
  ...
  return (
    <div className="flex-1 p-6 bg-black text-white overflow-hidden flex flex-col relative">
      ...
      <div className="bg-zinc-900/40 rounded-2xl overflow-hidden flex-1 flex flex-col min-h-0 backdrop-blur-sm relative z-10 shadow-2xl">
        <div className="overflow-auto custom-scrollbar flex-1">
        <table className="w-full text-sm">
          <thead className="bg-gradient-to-r from-zinc-900 to-zinc-800 text-zinc-400 sticky top-0 z-10">
                  ...
                  <td className="p-4">
                      ...
                      <div className="mt-2 flex gap-2">
                        {(() => {
                          let count = 0;
                          try {
                            const list = JSON.parse(localStorage.getItem(`comments_${v._id}`) || "[]");
                            count = Array.isArray(list) ? list.length : 0;
                          } catch (e) { void e; }
                          return (
                            <span className="px-2 py-0.5 rounded-full text-[10px] bg-zinc-800 text-zinc-300 border border-zinc-700">Comments {count}</span>
                          );
                        })()}
                      </div>
                  </td>
