...
// no view recording here; views counted on WatchPage
...
function TimeGapMeta({ video }) {
  ...
  }, [video.createdAt, video.created_at, video.uploadedAt, video.timestamp]);
  const label = useMemo(() => {
    if (!createdMs) return "";
  ...
  const likes = useMemo(() => {
    try {
      const map = JSON.parse(localStorage.getItem("likeCounts") || "{}");
      return Number(map[video._id] || 0);
    } catch {
      return 0;
    }
  }, [video._id]);
...
function VideoCard({ video, onOpen }) {
  ...
  const handleMouseEnter = () => {
      setIsHovering(true);
      // Do not record views on hover; views are counted on WatchPage only per account
  };
      ...
      <div className="thumb relative aspect-video bg-zinc-900 rounded-xl overflow-hidden">
        ...
        <div className="absolute top-2 right-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity">
           ...
           <AnimatePresence>
             {showMenu && (
               <div 
                 className="absolute right-0 top-full mt-2 w-32 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl overflow-hidden flex flex-col z-40 animate-in fade-in slide-in-from-top-1"
