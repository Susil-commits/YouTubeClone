...
router.post("/videos/:id/view", async (req, res) => {
  try {
    ...
    const vid = id;
    const exists = await ViewLog.findOne({ videoId: vid, userId: uid });
    let views;
    if (!exists) {
      try {
        await ViewLog.create({ videoId: vid, userId: uid });
      } catch {}
      const updated = await Video.findByIdAndUpdate(vid, { $inc: { "stats.views": 1 } }, { new: true });
      if (!updated) return res.status(404).json({ error: "not_found" });
      views = updated.stats?.views || 0;
    } else {
      const v = await Video.findById(vid).lean();
...
// Comments endpoints (server-backed comments without changing existing layouts)
router.get("/videos/:id/comments", async (req, res) => {
  try {
    const { id } = req.params;
    const list = await Comment.find({ videoId: id }).sort({ createdAt: -1 }).lean();
    res.json(list.map(c => ({ id: c._id, authorName: c.authorName || "Guest User", text: c.text || "", createdAt: c.createdAt })));
  } catch (e) {
    res.status(400).json({ error: "bad_request" });
  }
});
...
router.post("/videos/:id/comments", async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.id).lean().catch(() => null);
    const authorName = user?.name ? "Guest User" : "Guest User";
    const doc = await Comment.create({ videoId: id, userId: req.user.id, authorName, text });
    res.status(201).json({ id: doc._id, authorName: doc.authorName, text: doc.text, createdAt: doc.createdAt });
  } catch (e) {
    res.status(400).json({ error: "bad_request" });
  }
});
